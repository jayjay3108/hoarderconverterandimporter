...
def format_for_hoarder(data, list_name):
    ...
    for entry in data:
        if 'url' in entry and validate_url(entry['url']):
            formatted.append({
                'url': str(entry['url']) if entry.get('url') is not None else "",
                'title': str(entry.get('title', "")) if entry.get('title') is not None else "",
                'description': str(entry.get('description', "")) if entry.get('description') is not None else "",
                'tags': entry.get('tags', []),
                'list': str(list_name) if list_name is not None else "",
                'createdAt': now
            })
    return formatted
